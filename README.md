...
Product-led companies collect thousands of product events every day, but raw event data isn't something sales teams can act on. This project demonstrates how behavioral data can be transformed into qualified accounts by combining product analytics, CRM automation, company enrichment, and lead scoring into a single pipeline.
What it does
...
Free
$0
1 data source, 3 dashboards, 1 seat, 30-day history
Team
$49/seat/month
Unlimited data sources and dashboards, 10 seats, scheduled reports
Business
$99/seat/month
SSO, audit logs, row-level permissions, embedded analytics
...
Roughly 28,000 events get generated and sent to PostHog, backdated so it looks like a month and a half of real activity.
...
The lesson buried in this one: I first matched people by an internal ID that regenerated every run, so HubSpot saw everyone as new every time and piled up duplicates. The fix was to match on email, which doesn't change. I know it is obvious in hindsight, but it gave me a headache on initial reruns.
03_enrich_companies.py — Add company context
Generate_events.py tells you someone's interest from their behavior. But it doesn't tell you whether they're worth selling to. A power user at a five-person startup probably won't pay enterprise prices. At their size, a free or cheaper tool does the job fine. Meanwhile, a big company that's barely touched the product could still be the biggest deal of the quarter.
...
For the lookups, I included live integrations to Apollo, Hunter, and BuiltWith. I couldn't pay for them at this stage, though, and my companies are made up anyway, so the lookups come back empty. When that happens, the script fills in believable details that match each company's personality and tags every record so you can tell what's real, what's partly real, and what's invented.
...
The real fix would be to generate a more realistic mix of users rather than forcing the scoring model to compensate for unrealistic input.
...
Create a plait_env.sh file (gitignored) containing your API credentials:
...
It took me longer than I’d like to admit to figure that one out.
...
ifeomaorika@Ifeomas-MacBook-Pro what-sales-sees %
cd ~/what-sales-sees
mkdir images
